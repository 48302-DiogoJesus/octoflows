...
def main():
    ...
    for task_id in dag._all_nodes.keys():
        ...
        task_data = 0
        total_time_downloading_data_ms += metrics.input_metrics.tp_download_inputs_time_ms
    ...
    sink_task_ended_timestamp_ms = (_sink_task_metrics.started_at_timestamp_s * 1000) + _sink_task_metrics.input_metrics.tp_download_inputs_time_ms + _sink_task_metrics.execution_time_ms + _sink_task_metrics.output_metrics.time_ms + _sink_task_metrics.total_invocation_time_ms # type: ignore
    ...
    for task_id, metrics in zip(dag._all_nodes.keys(), dag_metrics):
        ...
        end_time = relative_start_time + metrics.input_metrics.tp_download_inputs_time_ms + metrics.execution_time_ms + metrics.total_invocation_time_ms
    ...
    with tab_viz:
        ...
        with details_col:
            ...
            if st.session_state.selected_task_id:
                ...
                with col1:
                    total_task_handling_time = max(metrics.input_metrics.tp_download_inputs_time_ms, 0) + max(metrics.execution_time_ms, 0) + max(metrics.update_dependency_counters_time_ms, 0) + max(metrics.output_metrics.time_ms, 0) + max(metrics.total_invocation_time_ms, 0)
                    st.metric("Total Task Handling Time", f"{total_task_handling_time:.2f} ms")
                    st.metric("Dependencies Download Time", f"{metrics.input_metrics.tp_download_inputs_time_ms:.2f} ms")
    ...
    with tab_data:
        if dag_metrics:
            ...
            for task_metrics in dag_metrics:
                ...
                if task_metrics.input_metrics.tp_download_inputs_time_ms > 0:
                    throughput_mb = (task_metrics.input_metrics.downloadable_input_size_bytes / (task_metrics.input_metrics.tp_download_inputs_time_ms / 1000)) / (1024 * 1024)  # MB/s
                    speed_bytes_ms = task_metrics.input_metrics.downloadable_input_size_bytes / task_metrics.input_metrics.tp_download_inputs_time_ms  # bytes/ms
