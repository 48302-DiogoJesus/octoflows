...
def main():
    ...
    if selected_workflow != 'All' and matching_workflow_instances:
            ...
            if instance_data:
                ...
                # Add memory configuration performance analysis
                st.markdown("### Performance by Memory Configuration")
                
                # Define fixed memory steps
                MEMORY_STEPS = [128, 256, 512, 1024, 2048, 4096, 8192]
                
                # Extract memory configurations and metrics
                memory_data = []
                for instance in matching_workflow_instances:
                    if not instance.plan or not instance.tasks or not instance.tasks[0].metrics.worker_resource_configuration:
                        continue
                        
                    memory_mb = instance.tasks[0].metrics.worker_resource_configuration.memory_mb
                    
                    # Skip if memory configuration is not in our fixed steps
                    ...
                    memory_data.append({
                        ...
                        'Value': actual_execution,
                        'Planner': planner_name,
                        'Planner_Metric': f"{planner_name} - Execution Time"
                    })
                    ...
                    memory_data.append({
                        ...
                        'Planner': planner_name,
                        'Planner_Metric': f"{planner_name} - Download Time"
                    })
                    ...
                    memory_data.append({
                        ...
                        'Value': actual_upload,
                        'Planner': planner_name,
                        'Planner_Metric': f"{planner_name} - Upload Time"
                    })
                    
                    memory_data.append({
                        'Memory (MB)': memory_mb,
                        'Metric': 'Makespan',
                        'Value': actual_makespan_s,
                        'Planner': planner_name,
                        'Planner_Metric': f"{planner_name} - Makespan"
                    })
                
                if memory_data:
                    df_memory = pd.DataFrame(memory_data)
                    
                    # Create a complete grid of all combinations for proper line connections
                    all_combinations = pd.MultiIndex.from_product(
                        [MEMORY_STEPS, 
                         df_memory['Planner'].unique(), 
                         df_memory['Metric'].unique()],
                        names=['Memory (MB)', 'Planner', 'Metric']
                    ).to_frame(index=False)
                    
                    # Group by memory, planner, and metric to get average values
                    df_avg = df_memory.groupby(['Memory (MB)', 'Planner', 'Metric', 'Planner_Metric'])['Value'] \
                                    .mean() \
                                    .reset_index()
                    
                    # Merge with all combinations to ensure all points are shown
                    df_avg = all_combinations.merge(
                        df_avg,
                        on=['Memory (MB)', 'Planner', 'Metric'],
                        how='left'
                    )
                    
                    # Ensure Memory (MB) is treated as numeric
                    df_avg['Memory (MB)'] = pd.to_numeric(df_avg['Memory (MB)'])
                    
                    # Create line chart with connected lines for each planner+metric combination
                    fig = px.line(
                        df_avg, 
                        x='Memory (MB)', 
                        y='Value', 
                        color='Planner_Metric',
                        title='Performance Metrics by Memory Configuration',
                        labels={'Value': 'Time (seconds)', 'Planner_Metric': 'Planner - Metric'},
                        markers=True,
                        line_dash='Planner'
                    )
                    ...
                    fig.update_layout(
                        xaxis_title='Memory Configuration (MB)',
                        yaxis_title='Time (seconds)',
                        legend_title='Planner - Metric',
                        hovermode='x unified',
                        height=600,
                        showlegend=True,
                        xaxis={
                            'type': 'log',
                            'tickmode': 'array',
                            'tickvals': MEMORY_STEPS,
                            'ticktext': [f"{mb}MB" for mb in MEMORY_STEPS]
                        }
                    )
                    
                    # Add vertical lines for memory configurations
                    ...
                    st.warning("No memory configuration data available for visualization.")
