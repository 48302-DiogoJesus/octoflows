...
def edge_detect_chunk(chunk: np.ndarray) -> np.ndarray:
    ...
    img_edge = img_edge.resize((chunk.shape[1], chunk.shape[0]))  # match original chunk size
...
def sharpen_chunk(chunk: np.ndarray) -> np.ndarray:
    ...
    img_sharp = img_sharp.resize((chunk.shape[1], chunk.shape[0]))  # match original chunk size
...
result = final_img.compute(dag_name="image_transformer", config=WORKER_CONFIG)
