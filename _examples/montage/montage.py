...
def scan_fits(input_dir: Path, pattern: str):
    ...
    for f in files:
        ...
        try:
            # Create a temp file manually in the current directory
            tmp_path = Path.cwd() / f"tmp_fits_{os.getpid()}.fits"
            ...
            with fits.open(tmp_path) as hdul:
                ...
                rows.append((f, hdu.data.copy(), wcs))

            # Remove the temp file manually
            tmp_path.unlink()
